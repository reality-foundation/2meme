...
Consider 2meme's L1, which has a Hausdorff dimensionality of 2 \footnote{see Appendix} and corresponding to thermal capacity at constant pressure. Consider also 2meme's L0 which which is given by a self avoiding random walk which has Hausdorff dimension $\frac{4}{3}$ corresponding to thermal capacity at constant volume. The specific heat $\frac{C_P}{C_V}$ is thus $\frac{2}{\frac{4}{3}} = \frac{3}{2}$ the same fractal dimensionality as the graph of brownian motion for a monatomic ideal gas\footnote{https://journals.aps.org/pre/abstract/10.1103/PhysRevE.84.021113}.
...
\section{Appendix}
\subsection{The L1 Block Structure}

\begin{definition}[L1 Block]
An L1 block $B$ is a signed data structure consisting of:
\begin{itemize}
    \item A \textbf{parent list} $\mathrm{parent}(B) = \{P_1, P_2\}$, a non-empty list of exactly $k = 2$ references to previous blocks (tips).
    \item A \textbf{transaction set} $\mathrm{txs}(B)$, a non-empty set of signed transactions.
    \item A \textbf{proof set} $\mathrm{proofs}(B) = \{\sigma_1, \sigma_2, \sigma_3\}$, consisting of $p + 1 = 3$ signature proofs from the owner and $p = 2$ facilitator peers.
\end{itemize}
\end{definition}

Each block is therefore a \textbf{2-simplex} (triangle) in the space of validators, with edges to two parent 2-simplices, forming a directed acyclic graph $\mathcal{G} = (V, E)$ where:
\begin{itemize}
    \item $V$ is the set of all blocks,
    \item $E = \{(B, P) : P \in \mathrm{parent}(B)\}$ is the set of parent edges.
\end{itemize}

\subsection{Hausdorff Dimension of the L1 DAG}

We now compute the Hausdorff dimension of $\mathcal{G}$ as a metric space under the graph distance metric.

\begin{definition}[Graph Distance Metric]
For blocks $B_1, B_2 \in V$, define:
\[
d(B_1, B_2) = \min\{|P| : P \text{ is a path from } B_1 \text{ to } B_2 \text{ in } \mathcal{G} \cup \mathcal{G}^{-1}\}
\]
where $\mathcal{G}^{-1}$ includes reverse edges (child-to-parent and parent-to-child).
\end{definition}

\begin{definition}[DAG Parameters]
Let:
\begin{itemize}
    \item $n$ = number of active L1 validators at any given time
    \item $k = 2$ = number of parent references per block (tip count)
    \item $s = 3$ = number of signers per block (peers count + 1)
    \item $w = \lfloor n/s \rfloor$ = maximum concurrent blocks per consensus round (each consuming $s$ validators)
\end{itemize}
\end{definition}

\begin{lemma}[Depth Growth]
Along any chain of parent references, the number of distinct blocks reachable within distance $r$ in the backward direction satisfies:
\[
N_{\mathrm{depth}}(r) = \min\left(k^r, D\right) = \min\left(2^r, D\right)
\]
where $D$ is the total depth of the DAG. Due to DAG merging (blocks sharing parents), the effective growth is bounded by the width at each level:
\[
N_{\mathrm{depth}}(r) \leq \sum_{i=0}^{r} w = (r+1) \cdot w
\]
\end{lemma}

\begin{lemma}[Lateral Growth]
At a fixed depth level $\ell$, blocks are connected laterally through shared tips and shared validators. The number of blocks reachable within lateral distance $r$ at level $\ell$ satisfies:
\[
N_{\mathrm{lateral}}(r) \leq \min\left(\binom{s \cdot r}{s-1}, w\right)
\]
For small $r$ relative to the level width, tip sharing creates approximately linear lateral growth:
\[
N_{\mathrm{lateral}}(r) \approx c \cdot r
\]
where $c$ depends on the tip selection strategy.
\end{lemma}

\begin{theorem}[Local Hausdorff Dimension]
\label{thm:main}
The local Hausdorff dimension of the L1 block DAG $\mathcal{G}$ is:
\[
d_H^{\mathrm{local}}(\mathcal{G}) = 2
\]
\end{theorem}

\begin{proof}
Consider a ball $\mathcal{B}(B, r)$ of radius $r$ centered at block $B$ in the graph distance metric. This ball expands in two independent directions:

\textbf{Direction 1 (Depth):} Following parent/child edges, the ball covers approximately $r$ depth levels in each direction. Each level contributes blocks proportional to the width at that level.

\textbf{Direction 2 (Lateral):} At each depth level within the ball, shared tips and validators create lateral connections. The number of laterally reachable blocks at each level grows approximately linearly with $r$ (for $r$ smaller than the level diameter).

The total number of blocks in $\mathcal{B}(B, r)$ is:
\[
N(r) = \sum_{\ell = -r}^{r} N_{\mathrm{lateral}}(r - |\ell|) \approx \sum_{\ell = -r}^{r} c(r - |\ell|) = c \sum_{j=0}^{r} j = c \cdot \frac{r(r+1)}{2} \sim \frac{c}{2} r^2
\]

By the definition of Hausdorff dimension via ball growth:
\[
N(r) \sim r^{d_H} \implies d_H = 2
\]
\end{proof}

\begin{theorem}[Global Hausdorff Dimension]
For $r$ exceeding the lateral diameter of the DAG at any level (i.e., $r \gg \log w$), the Hausdorff dimension degrades to:
\[
d_H^{\mathrm{global}}(\mathcal{G}) = 1
\]
\end{theorem}

\begin{proof}
When $r$ exceeds the lateral diameter $D_{\mathrm{lat}} \approx O(\log w)$, the ball $\mathcal{B}(B, r)$ saturates each depth level it touches. The volume becomes:
\[
N(r) = \sum_{\ell=-r}^{r} w = (2r + 1) \cdot w \sim 2wr
\]
Since $w$ is constant (fixed network size), $N(r) \sim r$, giving $d_H = 1$.
\end{proof}

\begin{remark}
$C_P = 2$ (thermal capacity at constant pressure) is derived from the L1 structure, and $C_V = 4/3$ (thermal capacity at constant volume) from the L0 self-avoiding walk, giving the specific heat ratio:
\[
\gamma = \frac{C_P}{C_V} = \frac{2}{4/3} = \frac{3}{2}
\]
\end{remark}

Its local Hausdorff dimension $d_H = 2$ implies $C_P = 2$ because:

\begin{enumerate}
    \item The thermal capacity corresponds to the \emph{local} dimensionality of the state space, not the global topology.
    \item In the equipartition theorem, each quadratic degree of freedom contributes $\frac{1}{2}k_BT$ to the thermal energy. The number of effective degrees of freedom is determined by the local dimension.
    \item The L1 block DAG has local dimension 2 (Theorem~\ref{thm:main}), corresponding to 2 independent directions of information propagation: temporal (depth) and spatial (lateral/validator spread).
\end{enumerate}

Therefore, the thermodynamic fee model's exponent is $\frac{1-\gamma}{\gamma} = -\frac{1}{3}$.
