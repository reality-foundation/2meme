...
\If{$h \bmod i-1 == 0 \& n.mempool.size > n.mempool.t$}
...
\ElseIf{$h \bmod i-1 == 0 \& n.mempool.size > n.mempool.t$}
...
\Else {$n.mempool.size > n.mempool.t $}
...
\State $\foreach n \exist n.mempool | n.mempool.size = s \in \mathbb{N} \& n.mempool.triggersize = t \in \mathbb{N} $
\While{$true$}
\If{$n.mempool.size > n.mempool.t \&  e == True$}
	 \State $state space = entropy(N)$ \Comment{Entropy rate relative to GCS}
	 \State $probability space = walk (state space)$  \Comment{Montecarlo method}
